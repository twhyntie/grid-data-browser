...
def makeFrameImage(basename, pixels, outputpath):
    ...
    # Note that some lines here have been commented out to create the
    # frame with no borders or colour axis.
    ...
    figsize = 4.0 #max(radius*0.8, 3.0)

    ## The figure for the frame.
    #frfig = plt.figure(1, figsize=(figsize*1.27, figsize), dpi=150, facecolor='w', edgecolor='w')
    frfig = plt.figure(1, figsize=(figsize, figsize), dpi=128, facecolor='w', edgecolor='w')

    #frfig.patch.set_visible(False)

    ## The frame axes.
    frfigax = frfig.add_subplot(111, axisbg='#222222')
    #frfigax = frfig.add_axes([0,0, 256, 256])
    #frfigax.axis('off')
    frfigax.set_frame_on(False)
    frfigax.set_xticks([])
    frfigax.set_yticks([])

    # Add the frame background (blue).
    frfigax.add_patch(plt.Rectangle((0,0),w*s,h*s,facecolor='#82bcff', edgecolor='#82bcff'))
...
#    colorbar.ColorbarBase(colax,cmap=cmap,norm=colors.Normalize(vmin=0,vmax=col_max))
    ...
    for X, C in pixels.iteritems():
        x = X % 256; y = X / 256
        scaled_C = float(C)/float(col_max)
        frfigax.add_patch(plt.Rectangle((x*s,y*s),2,2,edgecolor=cmap(scaled_C),facecolor=cmap(scaled_C)))

    # Set the axis limits based on the cluster radius.
    b = 0 #3 # border

    frfigax.set_xlim([0 - b, (256*s) + b])
    frfigax.set_ylim([0 - b, (256*s) + b])

    #frfigax.spines['top'].set_visible(False)
    #frfigax.spines['bottom'].set_visible(False)
    #frfigax.spines['right'].set_visible(False)
    #frfigax.spines['left'].set_visible(False)
