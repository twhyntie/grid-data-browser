...
class Dataset:
    ...
    def __init__(self, foldername):
        ...
        for i, fn in enumerate(self.filenames):
            ...
            lg.debug(" * Format value is: %d" % (formatval))
    ...
    def dscFilesPresent(self):
        ...
        lg.debug(" *")
        lg.debug(self.dscfilenames.values())
        lg.debug(" *")
        ...
        for i, bn in self.datfilenames.iteritems():
            ...
            if not dscn in self.dscfilenames.values():
                ...
                lg.debug(" * Data file '%s' has a DSC file." % (bn))
    ...
    def getFrames(self, geo, **kwargs):
        ...
        for i, df in enumerate(self.dscfiles):
            ...
            frameargs = {\
                ...
                "disc"        : df.getDisc(), \
                "preamp"      : df.getPreamp(), \
                "buffanaloga" : df.getBuffAnalogA(), \
                "buffanalogb" : df.getBuffAnalogB(), \
                "hist"        : df.getHist(), \
                "thl"         : df.getTHL(), \
                "thlcoarse"   : df.getTHLCoarse(), \
                "vcas"        : df.getVcas(), \
                "fbk"         : df.getFBK(), \
                "gnd"         : df.getGND(), \
                "ths"         : df.getTHS(), \
                "biaslvds"    : df.getBiasLVDS(), \
                "reflvds"     : df.getRefLVDS(), \
                #
                "firmwarev"   : df.getFirmwareVersion(), \
                "hwtimermode" : df.getHwTimerMode(), \
                "interface"   : df.getInterface(), \
                "mpxclock"    : df.getMpxClock(), \
                "mpxtype"     : df.getMpxType(), \
                "nameandsn"   : df.getNameAndSerialNumber(), \
                "pixelmanv"   : df.getPixelmanVersion(), \
                "polarity"    : df.getPolarity(), \
                "tpxclock"    : df.getTpxClock(), \
                #
